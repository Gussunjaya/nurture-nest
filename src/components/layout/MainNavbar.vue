...
const openSubscriptionModal = () => (showSubscriptionModal.value = true)
...
<template>
  <nav class="bg-white shadow-sm sticky top-0 z-40">
    <div class="container mx-auto px-4 py-3 flex justify-between items-center flex-wrap gap-2">
      <RouterLink to="/" class="text-2xl font-bold text-green-700">🌿 NurtureNest</RouterLink>
      <div class="flex items-center gap-3 flex-wrap">
        <div v-if="isLoggedIn" class="flex items-center gap-2">
          <button
            @click="openProfileModal"
            class="flex items-center gap-2 bg-gray-100 hover:bg-gray-200 rounded-full px-3 py-1 transition-all"
          >
            <span class="text-sm hidden xs:inline">{{ userEmail }}</span>
    ...
    >
      ...
      <div class="relative bg-white rounded-2xl shadow-xl max-w-md w-full p-6 md:p-8 z-10">
        ...
        <div class="flex flex-col items-center">
          ...
          <h2 class="text-xl font-semibold text-green-700">{{ userName || 'Pengguna' }}</h2>
          <p class="text-gray-500 text-sm break-all text-center">{{ userEmail }}</p>
          <div class="mt-2">
          ...
          <div class="w-full mt-6 border-t pt-4">
            ...
            <button
              ...
              class="mt-4 w-full bg-gray-100 text-green-700 py-2 rounded-full hover:bg-gray-200 transition"
            >
              Kelola Langganan
            </button>
            <button
